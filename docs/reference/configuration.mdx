...
Exporter metadata expressed as semicolon-separated `key:value` pairs. `instance_uuid` is included in all export formats; the other resource columns are populated by the Arrow exporters.
...
Supply `instance_uuid` with the Postgres service's canonical UUID alongside the existing `instance_ubid` and server attributes:

```ini
pg_stat_ch.extra_attributes = 'instance_uuid:01234567-89ab-8ad0-9234-56789abcdef0;instance_ubid:pg04hmasw9ne4j8t5cy4tqkff1;server_role:primary'
```

The provisioner supplies the UUID; the producer does not derive it from the UBID. Native ClickHouse and Arrow exports include an `instance_uuid` string column, and ordinary OTLP exports include an `instance_uuid` log attribute. An omitted value produces an empty string.

The bundled ClickHouse migration adds `instance_uuid String DEFAULT ''` to `events_raw`. Apply it **before deploying the producer when using native ClickHouse export**: its named inserts require the column even when the UUID is unset. ArrowStream inserts can arrive before the column exists; ClickHouse ignores the extra field until the schema includes it. External schemas and materialized-view projections need their own updates to retain the UUID; previously ignored values are not backfilled.
...
Each ring slot is about 520 bytes on 64-bit platforms; query and error text live in the DSA string area rather than in the slot. The default capacity of 131072 events uses about **68 MB** for slots, on top of `pg_stat_ch.string_area_size` (64 MB by default) and an interner hash table sized to the same capacity. Adjust based on your query throughput and acceptable memory usage.
...
| Workload | QPS | Recommended capacity | Ring slot memory |
|---|---|---|---|
| Low traffic | < 1,000 | 8192 | ~4 MB |
| Medium traffic | 1,000 - 10,000 | 65536 | ~34 MB |
| High traffic | 10,000 - 100,000 | 131072 (default) | ~68 MB |
| Very high traffic | > 100,000 | 524288+ | ~273 MB+ |
...
Limits the size of each batch sent to ClickHouse. The background worker drains up to `batch_max` events per insert, then loops until the queue is empty or below the threshold.

Values above `pg_stat_ch.queue_capacity` are clamped: one drain pass cannot yield more events than the ring buffer holds. The dequeue buffer is sized to the events actually queued, roughly 4.6 KB each, so a full-queue drain at the default peaks near 600 MB in the worker.
