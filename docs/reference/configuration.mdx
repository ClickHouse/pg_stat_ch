...
Limits the size of each batch sent to ClickHouse. The background worker drains up to `batch_max` events per insert, then loops until the queue is empty or below the threshold. Larger backlogs drain as consecutive full batches within one cycle.

Values above `pg_stat_ch.queue_capacity` are clamped: one drain pass cannot yield more events than the ring buffer holds. The maximum bounds worker batch memory, roughly 5 KB per event, near 80 MB.
