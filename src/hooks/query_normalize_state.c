...
static PschNormalizeCacheKey MakeCacheKey(uint64 query_id) {
  PschNormalizeCacheKey key;
...
static void EnsureCacheInitialized(PschNormalizedQueryCache* cache) {
  int max_entries;

  if (cache->htab != NULL)
    return;

  max_entries = cache->max_entries > 0 ? cache->max_entries : psch_normalize_cache_max;
  PschInitNormalizedQueryCache(cache, max_entries);
}

static int ClampCachedQueryLen(int normalized_len) {
...
static void PushEntryToMru(PschNormalizedQueryCache* cache, PschNormalizeCacheEntry* entry) {
  dclist_push_head(&cache->lru, &entry->lru_node);
}

static void PromoteEntryToMru(PschNormalizedQueryCache* cache, PschNormalizeCacheEntry* entry) {
  dclist_delete_from(&cache->lru, &entry->lru_node);
  PushEntryToMru(cache, entry);
}

static char* CopyQueryText(PschNormalizedQueryCache* cache, const char* normalized_query,
                           int normalized_len) {
  char* copy;
...
static uint16 CopyEntryText(const PschNormalizeCacheEntry* entry, char* dst, Size dst_size) {
  Size len;
...
void PschInitNormalizedQueryCache(PschNormalizedQueryCache* cache, int max_entries) {
  ...
  cache->mcxt =
      AllocSetContextCreate(TopMemoryContext, "pg_stat_ch normalize cache", ALLOCSET_DEFAULT_SIZES);

  MemSet(&info, 0, sizeof(info));
  info.keysize = sizeof(PschNormalizeCacheKey);
  info.entrysize = sizeof(PschNormalizeCacheEntry);
  info.hcxt = cache->mcxt;
  cache->htab = hash_create("pg_stat_ch normalize cache", Min(max_entries, 256), &info,
...
static void EvictLru(PschNormalizedQueryCache* cache) {
  dlist_node* tail;
...
void PschRememberNormalizedQuery(PschNormalizedQueryCache* cache, uint64 query_id,
  ...
  entry = (PschNormalizeCacheEntry*)hash_search(cache->htab, &hkey, HASH_ENTER, &found);
  ...
  entry->normalized_query = CopyQueryText(cache, normalized_query, normalized_len);
...
bool PschLookupNormalizedQuery(PschNormalizedQueryCache* cache, uint64 query_id, char* dst,
  ...
  if (cache == NULL || cache->htab == NULL || dst == NULL || dst_size == 0 || out_len == NULL ||
      query_id == UINT64CONST(0))
    return false;

  hkey = MakeCacheKey(query_id);
  entry = (PschNormalizeCacheEntry*)hash_search(cache->htab, &hkey, HASH_FIND, NULL);
  if (entry == NULL)
    return false;
