...
char* PschNormalizeQuery(const char* query, int query_loc, int* query_len_p, JumbleState* jstate) {
  ...
  // expects the parsing context to be short-lived).  We are called from
  // TopMemoryContext, so without this the per-query scanner allocations
  // would accumulate for the lifetime of the backend.
  {
    MemoryContext tmp =
