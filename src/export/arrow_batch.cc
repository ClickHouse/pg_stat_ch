...
bool AppendString(arrow::StringBuilder* builder, std::string_view value, const char* context) {
  ...
  const arrow::Status status =
      builder->Append(data, static_cast<int32_t>(value.size()));
  if (!status.ok()) {
    LogArrowFailure(context, status);
    return false;
  }
...
const char* CmdTypeToString(PschCmdType cmd_type) {
  switch (cmd_type) {
...
struct ArrowBatchBuilder::Impl {
  ...
  bool Append(const PschEvent& event) {
    ...
    const auto username_len = ClampFieldLen(event.username_len, static_cast<uint8>(sizeof(event.username)),
                                            "username_len");
    const auto app_len = ClampFieldLen(event.application_name_len,
                                       static_cast<uint8>(PSCH_MAX_APP_NAME_LEN),
                                       "application_name_len");
    const auto query_len =
        ClampFieldLen(event.query_len, static_cast<uint16>(PSCH_MAX_QUERY_LEN), "query_len");
    const auto err_message_len = ClampFieldLen(event.err_message_len,
                                               static_cast<uint16>(PSCH_MAX_ERR_MSG_LEN),
                                               "err_message_len");
    ...
    if (!AppendString(&query_id_builder, std::to_string(event.queryid), "Arrow query_id append") ||
        !AppendString(&db_name_builder, db_name, "Arrow db_name append") ||
        !AppendString(&db_user_builder, db_user, "Arrow db_user append") ||
        !AppendString(&db_operation_builder, CmdTypeToString(event.cmd_type),
        ...
        !AppendString(&pid_builder, std::to_string(event.pid), "Arrow pid append") ||
    ...
    if (!AppendScalar(&err_elevel_builder, static_cast<int32_t>(event.err_elevel),
        ...
        !AppendScalar(&shared_blk_read_time_us_builder,
                      ClampSignedToUint64(event.shared_blk_read_time_us,
                                          "shared_blk_read_time_us"),
                      "Arrow shared_blk_read_time_us append") ||
        !AppendScalar(&shared_blk_write_time_us_builder,
                      ClampSignedToUint64(event.shared_blk_write_time_us,
        ...
        !AppendScalar(&parallel_workers_planned_builder,
                      ClampSignedToUint32(event.parallel_workers_planned,
                                          "parallel_workers_planned"),
                      "Arrow parallel_workers_planned append") ||
        !AppendScalar(&parallel_workers_launched_builder,
                      ClampSignedToUint32(event.parallel_workers_launched,
    ...
    if (!AppendString(&instance_ubid_builder, ExtraAttr("instance_ubid"),
        ...
        !AppendString(&server_ubid_builder, ExtraAttr("server_ubid"),
                      "Arrow server_ubid append") ||
        !AppendString(&server_role_builder, ExtraAttr("server_role"),
...
bool ArrowBatchBuilder::Append(const PschEvent& event) { return impl_->Append(event); }

ArrowBatchBuilder::FinishResult ArrowBatchBuilder::Finish() { return impl_->Finish(); }

void ArrowBatchBuilder::Reset() { impl_->Reset(); }

int ArrowBatchBuilder::NumRows() const { return impl_->num_rows; }

size_t ArrowBatchBuilder::EstimatedBytes() const { return impl_->estimated_bytes; }
