...
class OTelExporter : public StatsExporter {
 ...
 private:
  ...
  shared_ptr<Column<uint64_t>> MakeDurationCol() {
    return std::make_shared<DurationCol>(this);
  }
  ...
  bool FlushChunk() {
    ...
    auto status = otlp::OtlpGrpcClient::DelegateExport(
        stub_.get(), std::move(context), std::move(arena_), request_, &response);
  ...
  // gRPC state
  otlp::OtlpGrpcClientOptions grpc_opts_;
