...
// Verify that fixed prefix of PschRingEntry has same binary layout as
// PschEvent up to (and including) query_len, last field before two
// variable-length char arrays / dsa_pointers diverge.  If field is added to
// PschEvent without updating PschRingEntry, these will fire at compile time.
// Use StaticAssertDecl so checks fire in both C and C++ translation units.
StaticAssertDecl(offsetof(PschEvent, ts_start) == offsetof(PschRingEntry, ts_start),
                 "ts_start offset mismatch");
StaticAssertDecl(offsetof(PschEvent, queryid) == offsetof(PschRingEntry, queryid),
                 "queryid offset mismatch");
StaticAssertDecl(offsetof(PschEvent, rows) == offsetof(PschRingEntry, rows),
                 "rows offset mismatch");
StaticAssertDecl(offsetof(PschEvent, shared_blks_hit) == offsetof(PschRingEntry, shared_blks_hit),
                 "shared_blks_hit offset mismatch");
StaticAssertDecl(offsetof(PschEvent, wal_records) == offsetof(PschRingEntry, wal_records),
                 "wal_records offset mismatch");
StaticAssertDecl(offsetof(PschEvent, err_sqlstate) == offsetof(PschRingEntry, err_sqlstate),
                 "err_sqlstate offset mismatch");
StaticAssertDecl(offsetof(PschEvent, err_message_len) == offsetof(PschRingEntry, err_message_len),
                 "err_message_len offset mismatch");
StaticAssertDecl(offsetof(PschEvent, application_name) == offsetof(PschRingEntry, application_name),
                 "application_name offset mismatch");
StaticAssertDecl(offsetof(PschEvent, query_len) == offsetof(PschRingEntry, query_len),
                 "query_len offset mismatch");
// Fields after query_len diverge: PschEvent has char arrays, PschRingEntry
// has dsa_pointers.  Intentional, fixed prefix up to query_len is
// block-copied with single memcpy.
StaticAssertDecl(offsetof(PschEvent, err_message) == offsetof(PschRingEntry, err_message_dsa),
